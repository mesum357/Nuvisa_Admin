...
export default function HeaderContentPage() {
  ...
  const fetchContents = async () => {
    setLoading(true);
    const response = await apiClient.get<HeaderContent[]>('/header-content');
  ...
  const handleUpdateContent = async (key: string) => {
    setSaving(true);
    const response = await apiClient.patch('/header-content', {
      key,
  ...
  const handleCreateContent = async () => {
    ...
    const section = prompt('Enter section (banner, navigation, contact, etc.):') || 'general';
    const type = prompt('Enter type (text, url, email, phone):') || 'text';
    const order = parseInt(prompt('Enter order (number):') || '0');

    setSaving(true);
    const response = await apiClient.post('/header-content', {
      key,
      value,
      type,
      section,
  ...
  const handleDeleteContent = async (id: string) => {
    ...
    setSaving(true);
    const response = await apiClient.delete(`/header-content?id=${id}`);
  ...
  const contentSections = [
    { title: 'Banner Content', section: 'banner' },
    { title: 'Navigation Content', section: 'navigation' },
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Manage header content and settings
          </p>
        </div>
